...
module Location : sig
  ...
  let print_loc ppf loc =
    ...
    if file = "//toplevel//" then begin
      ...
      Format.fprintf ppf "%s@{<loc>%a%s%i" msg_file print_filename file msg_line line;
      if startchar >= 0 then
  ...
  let print ppf loc =
    (* Hardcoded to the default printer *)
  ...
  let rec report_error ppf {loc; msg; sub; _} =
    ...
    List.iter (Format.fprintf ppf "@,@[<2>%a@]" report_error) sub;
    Format.fprintf ppf "@]"
